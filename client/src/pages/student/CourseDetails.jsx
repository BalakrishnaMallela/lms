...
const CourseDetails = () => {
  const {id} = useParams();
  const {allCourses,currency,calculateRatings,calculateChapterTime,calculateCourseDuration,calculateNoOfLectures} = useContext(AppContext);
        ...
        text-course-deatails-heading-small font-semibold text-gray-800">
          {courseData.courseTitle}
            ...
            dangerouslySetInnerHTML={{__html :courseData.courseDescription.slice(0,200)}}></p>
             ...
             <div className="flex">
               {[...Array(5)].map((_,i)=>(<img key={i}src={i < Math.floor(calculateRatings(courseData))? assets.star : assets.star_blank} alt="" className="w-3.5 h-3.5"/>))}
             ...
             <p className="text-blue-600">({courseData.courseRatings.length}{courseData.courseRatings.length > 1 ? "ratings" : "rating"})</p>
              ...
              <p className="text-sm">Course by <span className="text-blue-600 underline">katpadi Saicharan</span></p>
          ...
          <div className="pt-5">
            {
              courseData.courseContent.map((chapter,index)=>(
                <div key={index} className="border bordeer-gray-300 bg-white mb-2 rounded">
